...
fn lexer_rules() -> LexerRules {
    santiago::lexer_rules!(
        ...
        "DEFAULT" | "NUMBER"   = pattern r"[a-z0-9]+";
    )
}
...
fn grammar() -> Grammar<AST> {
    santiago::grammar!(
        ...
        "loop" => rules "REPEAT" "number" "command" => |nodes: Vec<AST>| {
            let n = if let AST::Number(val) = nodes[1] { val } else { unreachable!(); };
        ...
        "number" => lexemes "NUMBER" => |lexemes| {
            AST::Number(lexemes[0].raw.parse::<i32>().expect("c est pas un i32 !!!!"))
        };
    )
}
...
pub fn main() -> std::io::Result<()> {
    ...
    let lex_rules = lexer_rules();
    let lexemes = santiago::lexer::lex(&lex_rules, input).expect("lexem non trouvé !!!");
    let grammar = grammar();
